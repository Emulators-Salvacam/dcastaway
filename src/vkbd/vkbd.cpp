...
{
	vkbd_button2=vkbd_button3=vkbd_button4=(SDLKey)0;
}
...
int vkbd_init(void)
{
	...
	SDL_FreeSurface(tmp);
	tmp=IMG_Load(DATA_PREFIX "mouse.png");
	if (tmp==NULL)
	{
...
void vkbd_mouse(void)
{
	...
	r.w=kmou->w;
	r.h=kmou->h;
	SDL_BlitSurface(kmou,NULL,screen,&r);
}
...
#ifdef ALWAYS_LOW
	r.y=screen->h-40+vkbd_rect[vkbd_actual].rect.y;
#else
	r.y=screen_height-40+vkbd_rect[vkbd_actual].rect.y;
#endif
